...
\item Creates a `GridTiler` instance to place all of the cells.
\end{enumerate}
...
Sample code for drawing a via is shown in \ref{fig:example-via-code}. The precharge cell
after drawing all vias and the power strap is shown in \ref{fig:precharge-vias}.
...
\item Place the standard cells, often using a `GridTiler` (\ref{sec:placement-utilities}) with tiles taking their bounding box from a "PR boundary" layer.
